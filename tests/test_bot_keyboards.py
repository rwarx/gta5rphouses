...
def test_sub_kind_markup_encodes_server_and_kind(bot):
    ...
    assert "sub:add:20:house" in cbs
    assert "sub:add:20:apartment" in cbs
    assert "menu:subpick" in cbs  # back


def test_sub_add_callback_parses_into_four_parts(bot):
    # Mirrors the split in _on_callback: `_, _, sid, kind = data.split(":", 3)`
    data = "sub:add:20:apartment"
    _, _, sid, kind = data.split(":", 3)
    assert sid == "20"
    assert kind == "apartment"


def test_sub_del_callback_parses_sid(bot):
    data = "sub:del:20"
    sid = data.split(":", 2)[2]
    assert sid == "20"
